...
import type { BarcodeConfig } from '../App'; // adjust path if needed

/**
 * Generates a PDF417 barcode as a PNG data URL using a canvas for browser compatibility.
 * @param data The string data to encode in the barcode.
 * @param config Optional barcode dimension and error correction settings.
 * @returns A promise that resolves with the PNG data URL of the barcode.
 * @throws If barcode generation fails.
 */
export const generatePdf417Barcode = async (
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Build the PDF417-specific options string from config
      // bwip-js accepts these as a space-separated string in the `options` field
      const eclevel = config?.errorCorrectionLevel ?? 5;
      const columns = config?.columns ?? 10;
      const rows    = config?.rows && config.rows > 0 ? config.rows : null;

      let pdf417Options = `eclevel=${eclevel} columns=${columns}`;
      if (rows) pdf417Options += ` rows=${rows}`;

      const options: bwipjs.ToBufferOptions = {
        bcid:        'pdf417',
        text:        data,
        scale:       config?.width  ?? 3,   // px per module (bar width)
        height:      config?.height ?? 25,  // bar height in mm
        includetext: false,
        textxalign:  'center',
      ...
      if (
        errorMessage.toLowerCase().includes('value too long') ||
        errorMessage.toLowerCase().includes('data too large') ||
        errorMessage.toLowerCase().includes('capacity exceeded')
      ) {
        errorMessage =
          'The provided data is too long for the PDF417 barcode with current settings. ' +
          'Try reducing data, increasing columns, or lowering the error correction level.';
      } else if (errorMessage.toLowerCase().includes('unknown eci')) {
        errorMessage =
          'The input data may contain characters not supported by the current PDF417 ' +
          'encoding mode. Please verify input data, especially special characters.';
      } else if (errorMessage.toLowerCase().includes('bad ecc')) {
        errorMessage =
          'Invalid error correction level specified. PDF417 supports levels 0–8.';
      } else if (errorMessage.toLowerCase().includes('rows')) {
        errorMessage =
          'Invalid row count for the given data and column settings. ' +
          'Try setting Rows to 0 (auto) or choosing a higher value.';
      }
